...
|---|---|---|
| dsh installed | `dsh --version` | a version like `0.1.2-alpha.1` or newer |
...
2. Append (adjust the path to the real checkout location):
   ...
   duplicate ids — dsh `0.1.2-alpha.1` fails the boot with
...
>   from model or attachment content. On timeout the child is terminated
>   (SIGTERM→SIGKILL, `taskkill /t /f` on Windows) and awaited before cleanup.
