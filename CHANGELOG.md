...
- **Removed Dead Repository Artifacts / 清理死产物与冗余文件**:
  - Removed committed legacy tarballs (`dsh-tesseract-ocr-0.5.0.tgz`, `dsh-tesseract-ocr-0.6.0.tgz`) and redundant `package-lock.json` from git tracking. / 从版本库追踪中彻底移除了遗留的历史 tarball 产物包（0.5.0、0.6.0）以及多余的 `package-lock.json`。
...
- **Verified against deepseek-harness `0.1.3-alpha.1` (latest release).** No seam changes since `0.1.2-rc.1`: the `ctx.llm.resolveModelInfo` shim and the `agent/pre-step` message rewrite remain the stable integration points, and `ctx.attachments.readImage` is source-identical between the two tags (0.1.3's new file-attachment types are additive and untouched by this plugin). The vendored `@deepseek-ai/cordis` `4.0.2` and the loader/bundle patch mechanism are unchanged, so no code migration is required. 0.1.3's headline changes (environment-proxy support, Session persistence rework) do not touch any service this plugin consumes. README/README.zh-CN references updated; tests ALL PASS. / **已在 deepseek-harness `0.1.3-alpha.1` 最新发行版上验证。** 自 `0.1.2-rc.1` 以来无缝接口变更：`ctx.llm.resolveModelInfo` 能力补丁 + `agent/pre-step` 消息改写仍为稳定集成点，`ctx.attachments.readImage` 在两个 tag 之间源码完全一致（0.1.3 新增的文件附件类型为纯增量，本插件不涉及）。内置 `@deepseek-ai/cordis` `4.0.2` 与 loader / bundle 补丁机制均未变化，无需代码迁移。0.1.3 的主要变更（环境代理支持、Session 持久化重构）均不涉及本插件消费的任何服务。README/README.zh-CN 引用已更新；测试全部通过。

## [0.3.7] - 2026-09-03

### Compatibility / 兼容性

- **Verified against deepseek-harness `0.1.2-rc.1` (latest `master`).** No seam changes since `0.1.2-alpha.5` (`ctx.llm.resolveModelInfo` shim + `agent/pre-step` rewrite remain the stable integration points); the vendored `@deepseek-ai/cordis` `4.0.2` and the loader/bundle patch mechanism are unchanged, so no code migration is required. README/README.zh-CN references updated; tests ALL PASS. / **已在 deepseek-harness `0.1.2-rc.1` 最新 `master` 上验证。** 自 `0.1.2-alpha.5` 以来无缝接口变更（`ctx.llm.resolveModelInfo` 能力补丁 + `agent/pre-step` 改写仍为稳定集成点）；内置 `@deepseek-ai/cordis` `4.0.2` 与 loader / bundle 补丁机制均未变化，无需代码迁移。README/README.zh-CN 引用已更新；测试全部通过。

## [0.3.6] - 2026-09-02

### Compatibility / 兼容性

- **Verified against deepseek-harness `0.1.2-alpha.5` (latest `master`).** No seam changes since `0.1.2-alpha.4` (`ctx.llm.resolveModelInfo` shim + `agent/pre-step` rewrite remain the stable integration points). README/README.zh-CN references updated; no code migration required. / **已在 deepseek-harness `0.1.2-alpha.5` 最新 `master` 上验证。** 自 `0.1.2-alpha.4` 以来无缝接口变更（`ctx.llm.resolveModelInfo` 能力补丁 + `agent/pre-step` 改写仍为稳定集成点）。README/README.zh-CN 引用已更新；无需代码迁移。
...
- **Adapted to deepseek-harness `0.1.2-alpha.3` (master)**: between `0.1.2-alpha.2`
  ...
  builds against). `lib/` is rebuilt; the mock-based standalone test still passes.
...
- The plugin now exports the documented `Config` Schemastery schema
  ...
  fills defaults before `apply()` runs; invalid config fails the boot loudly
  instead of being silently ignored. `@deepseek-ai/schemastery` `3.18.2` is
  pinned and a `@deepseek-ai/cordis` peer range (`>=4.0.0 <5.0.0`) declares
  the supported framework floor.
- 插件现在按文档导出 `Config` Schemastery schema
  （docs/user/develop/basic/config）：加载器在 `apply()` 之前校验配置并填充
  默认值，非法配置会大声地启动失败，而不再被静默忽略。锁定
  `@deepseek-ai/schemastery` `3.18.2`，并新增 `@deepseek-ai/cordis` 的
  peer 区间（`>=4.0.0 <5.0.0`）以声明所支持的框架下限。
...
- **npm package renamed back to unscoped `dsh-tesseract-ocr`** (dropped the
...
- **Duplicate loader entry id trap**: dsh `0.1.0-rc.7` (cordis-plugin-loader
  ...
  boot with `duplicate loader entry id: tesseract-ocr`. The docs previously
  let users install the plugin **both** as an npm bundle **and** as a manual
  `cordis.patch.yml` insert (same id, twice). Install modes are now documented
  as mutually exclusive.
- **Broken config-override guidance**: agents-install told users to modify an
  existing row with `- update:`, a patch form that is not part of the dsh
  patch dialect and is silently skipped. The docs now show the correct
  id-targeted override row (`- id: tesseract-ocr` + `config:`), which replaces
  the existing row's config without registering a second entry.

## [0.2.2] - 2026-08-17

### Changed

- **npm release + Trusted Publishing**: first npm release (`@maxwell-feng/dsh-tesseract-ocr`, public, `0.2.2`); future releases publish from GitHub Actions `publish.yml` via OIDC with automatic provenance — no long-lived npm tokens.

## [0.2.1] - 2026-08-16

First tagged release. Addresses the community review blockers from
[dshoneys/awesome-dshoneys#1](https://github.com/dshoneys/awesome-dshoneys/issues/1).
...
- **Awaited child cleanup**: on timeout/teardown the OCR child process is
  terminated and awaited (SIGTERM→SIGKILL, `taskkill /t /f` on Windows)
  before the temp directory is removed; removal failures retry once and are
  logged instead of silently swallowed.
- **Quote-aware `tesseractBin`**: paths with spaces such as
  `"C:\Program Files\Tesseract-OCR\tesseract.exe"` are parsed correctly while
  unquoted prefix args still work.
...
- Package renamed to the scoped `@maxwell-feng/dsh-tesseract-ocr`.
- Dev dependencies pinned; `package-lock.json` committed.
- Added GitHub Actions CI: `npm ci` + 18 standalone tests + real Tesseract
  smoke test on Ubuntu.
