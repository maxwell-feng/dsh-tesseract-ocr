...
async function removeTempDir(dir, warn) {
    try {
        ...
        warn?.("[tesseract-ocr] temp dir remove failed (retrying): %s (%s)", dir, error instanceof Error ? error.message : String(error));
    }
    await sleep(200);
    try {
        await fs.rm(dir, { recursive: true, force: true });
        ...
        warn?.("[tesseract-ocr] temp dir remove failed: %s (%s)", dir, error instanceof Error ? error.message : String(error));
    }
}
export function apply(ctx, config = {}) {
    ...
    const tesseractBin = typeof config.tesseractBin === "string" && config.tesseractBin.length > 0
        ? config.tesseractBin
        : "tesseract";
    ...
    async function ocrText(ref) {
        ...
        pending = (async () => {
            ...
            ocrCache.delete(key);
            ctx.logger?.warn?.("[tesseract-ocr] OCR failed for %s: %s", key, error instanceof Error ? error.message : String(error));
    ...
    async function rewriteMessages(messages, agent) {
