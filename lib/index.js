...
export function apply(ctx, config = {}) {
  ...
  const tesseractBin =
    typeof config.tesseractBin === "string" && config.tesseractBin.length > 0
      ? config.tesseractBin
      : "tesseract";
  const psm = Number.isInteger(config.psm) && config.psm >= 0 ? config.psm : 3;
  const timeoutMs =
  ...
  async function ocrText(ref) {
    ...
    pending = (async () => {
      ...
      ocrCache.delete(key);
      ctx.logger?.warn?.("[tesseract-ocr] OCR failed for %s: %s", key, error?.message ?? String(error));
  ...
  async function runOcr(bytes, mediaType) {
    const dir = await fs.mkdtemp(join(tmpdir(), "tesseract-ocr-"));
