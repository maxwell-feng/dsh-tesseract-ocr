// Standalone pipeline test for dsh-tesseract-ocr (pure TypeScript).
import test from "node:test";
import assert from "node:assert/strict";
import { fileURLToPath } from "node:url";
import { apply, parseCommandSpec } from "../src/index.ts";

const MOCK = fileURLToPath(new URL("./mock-tesseract.ts", import.meta.url));
const NODE = process.execPath;
function quoteArg(value: string | number) {
  return `"${String(value).replaceAll('"', '\\"')}"`;
}

// Quote both node and mock path, and include --experimental-strip-types for running the ts mock
const MOCK_BIN = `${quoteArg(NODE)} --experimental-strip-types ${quoteArg(MOCK)}`;
...
test("dsh-tesseract-ocr pipeline suite", async () => {
  // --- parseCommandSpec -------------------------------------------------------
  {
    const spaced = parseCommandSpec(MOCK_BIN);
    assert.equal(spaced[0], NODE, "parseCommandSpec keeps quoted binary intact");
    assert.equal(spaced[1], "--experimental-strip-types");
    assert.equal(spaced[2], MOCK, "parseCommandSpec keeps quoted mock path");

    const winPath = String.raw`"C:\Program Files\Tesseract-OCR\tesseract.exe"`;
    const win = parseCommandSpec(winPath);
    assert.equal(win.length, 1);
    assert.equal(win[0], String.raw`C:\Program Files\Tesseract-OCR\tesseract.exe`);

    const withArgs = parseCommandSpec("node ./mock.ts --flag");
    assert.equal(withArgs[0], "node");
    assert.equal(withArgs[1], "./mock.ts");
    assert.equal(withArgs[2], "--flag");
  }
  ...
  apply(ctx as any, { language: "eng", tesseractBin: MOCK_BIN });
  ...
  // missing attachment is fail-closed
  {
    ...
    apply(ctxMissing as any, { tesseractBin: MOCK_BIN });
  ...
  // default: vision models also OCR
  {
    ...
    apply(ctxVd as any, { tesseractBin: MOCK_BIN });
  ...
  // opt-in vision passthrough
  {
    ...
    apply(ctx2 as any, { tesseractBin: MOCK_BIN, passthrough: true });
  ...
  // passthrough=true still OCRs text-only models
  {
    ...
    apply(ctx3 as any, { tesseractBin: MOCK_BIN, passthrough: true });
  ...
  // reject decisions pass through untouched
  {
    ...
    apply(ctxR as any, { tesseractBin: MOCK_BIN });
