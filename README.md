...
Tested on Ubuntu (primary target); works anywhere the `tesseract` CLI is installed (Linux, macOS, Windows). Verified against dsh `0.1.5-rc.2`.
...
> `- insert:` row with the same id to your profile — dsh `0.1.5-rc.2`
...
> and dsh `0.1.5-rc.2` fails the boot with `duplicate loader entry id: tesseract-ocr`
