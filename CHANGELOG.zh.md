...
- **清理仓库死产物与冗余文件**：
  - 从版本控制中彻底移除了遗留的历史 tarball 产物包（`dsh-tesseract-ocr-0.5.0.tgz`、`dsh-tesseract-ocr-0.6.0.tgz`）以及冗余的 `package-lock.json`（统一使用 pnpm）。
- **文档与 0.1.5-rc.2 同步**：
...
- **纯 TypeScript 架构重构（零 JavaScript 残留）**：
  ...
  - 移除了所有旧的 `.mjs` / `.js` 文件，包括将测试 mock 统一改造为 `test/mock-tesseract.ts`；
